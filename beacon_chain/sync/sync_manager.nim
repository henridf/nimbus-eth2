...
proc getBlocks*[A, B](man: SyncManager[A, B], peer: A,
  ...
  try:
    ...
    if res.isErr():
      debug "eip4844 epoch", epoch = man.cfg.EIP4844_FORK_EPOCH
  ...
  try:
    ...
    let blobData =
      if req.slot.epoch >= man.cfg.EIP4844_FORK_EPOCH and
        ...
        debug "not gettingblobs", epoch=req.slot.epoch, eip4844_epoch=man.cfg.EIP4844_FORK_EPOCH, diff=man.getLocalWallSlot().epoch - MIN_EPOCHS_FOR_BLOBS_SIDECARS_REQUESTS
    ...
    await man.queue.push(req, blockData, blobData, proc() =
